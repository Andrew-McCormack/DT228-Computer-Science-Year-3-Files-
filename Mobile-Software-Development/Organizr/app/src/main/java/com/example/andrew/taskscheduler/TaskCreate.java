...
public class TaskCreate extends AppCompatActivity
{
    ...
    private static final String DEBUG_TAG = "PHPTASKCREATION";
    public boolean finishedGettingOtherUserId;
    public Task newTask;
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        newTask.setUserId(bundle.getInt("userId"));
        newTask.setEventDetails(bundle.getString("eventDetails"));
        eventDate = bundle.getString("eventDate");
        newTask.setEventUrgent(bundle.getString("eventUrgency"));
        newTask.setEventDuration(bundle.getString("eventDuration"));
        newTask.setOtherUserId(Integer.parseInt(bundle.getString("otherUser")));
        otherUserName = bundle.getString("otherUser");

        String stringUrl = bundle.getString("stringUrl");

        finishedGettingOtherUserId = true;
...
// a string.
    private String downloadUrl(String myurl) throws IOException
    {
        ...
        try
        {
                ...
                String POST_PARAMS;
                if (newTask.getOtherUserId() == -1)
                {
                    POST_PARAMS = "userId=" + newTask.getUserId()
                            + "&" +
                            "eventDetails=" + newTask.getEventDetails()
                            + "&" +
                            "eventDate=" + eventDate
                            + "&" +
                            "eventUrgent=" + newTask.getEventUrgent()
                            + "&" +
                            "eventDuration=" + newTask.getEventDuration();
                }
                else
                {
                    POST_PARAMS = "userId=" + newTask.getUserId()
                            ...
                            "eventDuration=" + newTask.getEventDuration()
                            + "&" +
                            "otherUserId=" + newTask.getOtherUserId();
                }
